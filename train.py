...
def train_autoencoder(
    ...
    for epoch in range(params["n_epochs"]):
        ...
        for step, sample in enumerate(train_ds):
            (_, batch), _ = sample
            # batch shape (bins, height, width)
            batch = torch.unsqueeze(torch.from_numpy(batch), 1).to(device)

            generated_batch = model(batch)

            loss: torch.Tensor = criterion(batch, generated_batch)
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
