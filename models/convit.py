"""These modules are adapted from https://github.com/facebookresearch/convit/blob/main/convit.py"""
...
class CustomConViTEvents(pl.LightningModule):
    def __init__(
        self,
    ...
    def forward(self, x: torch.Tensor):
        x = self.convvit(x)
        bs = x.shape[0]
        x = x.reshape(bs, 4, 4, 32, 32)
        x = x.transpose(2, 3)
        x = x.reshape(bs, 1, 128, 128)
...
class CustomConViTEventsB(pl.LightningModule):
    def __init__(
        self,
        input_shape: Tuple[int, int, int],
        patch_size: Tuple[int, int],
...
class CustomConViTEventsC(pl.LightningModule):
    def __init__(
        ...
        self.conv_decoder = torch.nn.Sequential(
            ...
            # torch.nn.BatchNorm2d(128),
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(128, 64, 2, 2, padding=0),
            # torch.nn.ConvTranspose2d(128, 64, 3, padding=1),
            # torch.nn.BatchNorm2d(64),
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64, 64, 2, 2, padding=0),
            ...
            # torch.nn.BatchNorm2d(64),
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64, 32, 2, 2, padding=0),
            # torch.nn.ConvTranspose2d(64, 32, 3, padding=1),
            # torch.nn.BatchNorm2d(32),
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(32, 32, 2, 2, padding=0),
